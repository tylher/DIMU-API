...
public class AgreementServiceImpl implements AgreementService {
    ...
    public ApiResponseDto completeTransaction(User user, String agreementId) {
        try {
            ...
            DiimuWallet wallet = walletRepository.findByUser(user).getFirst();
            if (user.getUserId().equals(agreement.getSeller().getUserId()) && goodServices.isDeliveryConfirmed()) {
                if (agreement.getTransaction().getStatus().equals(TransactionStatus.COMPLETED)) {
                    throw new CustomException("Transaction has already been completed");
                }
                wallet.setEscrowBalance(wallet.getEscrowBalance() - agreement.getAmount());
                wallet.setAccessibleBalance(wallet.getAccessibleBalance() + agreement.getAmount());
                wallet.setLedgerBalance(wallet.getLedgerBalance() + agreement.getAmount());
                walletRepository.save(wallet);
                agreement.getTransaction().setStatus(TransactionStatus.COMPLETED);
            } else {
                throw new CustomException("You are not not authorized to complete this transaction");
            }
            return new ApiResponseDto(true, "Transaction completed successfully", agreementRepository.save(agreement));
        } catch (ResourceNotFoundException e) {
            throw e;
        } catch (Exception e) {
            throw new CustomException("Unexpected error occurred while completing transaction: " + e.getMessage());
        }
    }
